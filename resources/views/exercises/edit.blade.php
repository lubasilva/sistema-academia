...
<div class="container-fluid px-3 px-md-4">
    ...
    <div class="mb-4">
        <div class="d-flex align-items-center mb-2">
            ...
            <div>
                <h1 class="h4 mb-0 fw-bold">Editar Exercício</h1>
                <p class="text-muted mb-0 small">{{ $exercise->name }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-8 mx-auto">
                    ...
                    <form method="POST" action="{{ route('exercises.update', $exercise) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('muscle_group') is-invalid @enderror" 
                                ...
                                @foreach($muscleGroups as $key => $label)
                                    <option value="{{ $key }}" {{ old('muscle_group', $exercise->muscle_group) == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      ...
                                      name="description" 
                                      rows="4"
                                      placeholder="Descreva como executar o exercício, músculos trabalhados, dicas de execução...">{{ old('description', $exercise->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Preview da Imagem Atual</label>
                            <div>
                        ...
                        <div class="mb-4">
                            <div class="form-check form-switch">
                                <input class="form-check-input" 
                                       ...
                                       name="is_active" 
                                       value="1"
                                       {{ old('is_active', $exercise->is_active) ? 'checked' : '' }}>
            ...
            <div class="alert alert-info mt-3">
                ...
                Este exercício está sendo usado em <strong>{{ $exercise->workoutExercises()->count() }}</strong> treino(s).
                Se desativá-lo, não aparecerá mais na lista de exercícios disponíveis para novos treinos.
            </div>
            @endif
        </div>
    </div>
